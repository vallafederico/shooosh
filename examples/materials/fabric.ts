/**
 * Example-owned static fabric material composer. No optional lobe imports.
 * composeFabric(sheenWgsl, "base + fabricSheen(n,l,v,rough) * uUni.values0.y")
 * Slots: 1 roughness, 2 sheen, 3 coat, 4 coat roughness, 5 weave, 6 light angle.
 * Procedural normal perturbation and ambient fill are artistic approximations;
 * this is not an energy-compensated layered BSDF or a cloth simulation.
 */
export const fabricPrefix = `
fn fsMain() -> vec4f {
  ...
  let lit: vec3f = `
export const fabricSuffix = `;
  let mapped: vec3f = lit / (vec3f(1.0) + lit);
  return vec4f(pow(mapped, vec3f(0.45454545)), 1.0);
}
`

export function composeFabric(functions = "", lighting = "base") {
  return functions + fabricPrefix + lighting + fabricSuffix
}
